...
export function WizHelperText(
  props: Partial<Omit<InputCommonProps, 'path'>> & {
    path: string | null;
    validatedOverride?: 'error' | undefined;
    errorOverride?: string;
  }
) {
  const { validated: contextValidated, error: contextError } = useInputValidation({
    ...props,
    path: props.path || '',
  });
  const validated = props.validatedOverride ?? contextValidated;
  const error = props.errorOverride ?? contextError;
